...
// The most general array operation... reduce.
...
// Arguments --
// 1: accumulator: gets assigned the return value of the callback from the previous iteration.
// 2: current: the 'current' element being processed.
// 3: index: the index of the current element.
// 4: the array on which reduce is called.

// an additional parameter exists which sets the initial value of the accumulator.
// if the second parameter is not used, the first iteration is skipped and the second iteration gets passed the first element of the array
// as the accumulator.
 ...
 * Note that the rating values are saved as strings in the object and
...
function getRating(watchList){
  ...
  const ratingSum = watchList.reduce((acc, movie) => {
    if (movie['Director'] === 'Christopher Nolan') {
      acc += parseFloat(movie['imdbRating']);
      count++;
    }
